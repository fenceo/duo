# Duo working agreement

- For each completed round of implementation/optimization, update the version and release notes, run the relevant tests, commit and push the source, and publish a new **stable GitHub Release** to `fenceo/duo`. A local build or pushed commit alone does not complete a release. This is the maintainer's standing preference; an explicit instruction for the current task takes precedence.
