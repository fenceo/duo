...
当前边界：Codex / Claude 的历史原生会话尚未完整持久化账号快照，排队运行也还不能宣称账号已锁定在提交时。切换账号应新建任务；旧任务需要续接时先切回原账号。Harness 会话有独立运行时约束。后续需要显式任务/运行账号快照、历史未知绑定的确认流程与并发回归，不能猜测旧会话归属或静默迁移凭据。
