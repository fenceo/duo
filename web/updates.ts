...
function installUpdates(){
 ...
 nav.querySelectorAll<HTMLButtonElement>('button').forEach(b=>b.onclick=()=>{const page=b.dataset.settings!;nav.querySelectorAll('button').forEach(x=>x.classList.toggle('selected',x===b));for(const id of ['environment','feishu','access','engines','updates']){const section=document.getElementById('settings-'+id);if(section)section.classList.toggle('hidden',id!==page)};button('settings-save').classList.toggle('hidden',page==='updates'||page==='engines');if(page==='updates')void loadUpdateInformation();if(page==='engines')void loadEngineSettings()});
...
function updateLinks(v:UpdateInfo):Array<[string,string|undefined]>{const links:Array<[string,string|undefined]>=[['版本页面',v.release_url||v.releases_url]];if(v.download_url&&!v.install_supported)links.unshift(['手动下载便携版'+(v.size?' · '+(v.size/1024/1024).toFixed(1)+' MB':''),v.download_url]);if(v.checksum_url)links.push(['校验文件',v.checksum_url]);return links}
