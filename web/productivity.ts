...
function installProductivity(){
 ...
 element('task-actions').insertAdjacentHTML('beforeend','<button id="task-pin">置顶</button><button id="task-archive">归档</button>');
 button('task-pin').onclick=()=>void changeTaskPreference('pinned');button('task-archive').onclick=()=>void changeTaskPreference('archived');
...
async function openSearchHit(hit:SearchHit){
 ...
 if(hit.kind==='note'){switchTab('note');input('note-content').focus()}
...
async function changeTaskPreference(field:'pinned'|'archived'){
 if(!detail||!chosen)return;const id=chosen,value=!detail.task[field];button(field==='pinned'?'task-pin':'task-archive').disabled=true;
 try{const t=await api<Task>('tasks/'+id+'/preferences','PATCH',{[field]:value});const i=tasks.findIndex(t=>t.id===id);if(i>=0)tasks[i]=t;if(id===chosen&&detail){detail.task=t;if(field==='archived')taskView=t.archived?'archived':'active';renderTask()}renderList();notify(field==='pinned'?(value?'任务已置顶':'已取消置顶'):(value?'任务已归档，记录保留，可随时恢复':'任务已恢复'));if(input('search').value.trim())void searchEverywhere()}catch(e){notify((e as Error).message)}finally{if(chosen===id)renderTask()}
}
