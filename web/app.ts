...
async function api<T=any>(path:string,method='GET',data?:unknown):Promise<T>{
 ...
 if(!response.ok){if(response.status===401&&path!=='login'){authenticated=false;showLogin()}throw Object.assign(new Error(result.error||'请求失败'),{status:response.status})}
 return result;
}
