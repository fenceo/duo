...
function renderShell(){
 ...
 installTools();installWorkspace();installConversationFilter();installSettingsSections();installProductivity();installTerminal();installExecution();installDiscovery();installLayout();installEnvironmentDiscovery();installWorkflow();installStickyBoard();installAppearance();installUpdates();
 ...
 input('create-environment').onchange=()=>void loadCreateEnvironment();input('create-model').onchange=()=>input('custom-model').classList.toggle('hidden',input('create-model').value!=='__custom__');button('reload-models').onclick=()=>loadCreateEnvironment(true);
...
function renderTask(){
 if(!detail)return;const t=detail.task;element('task-title').textContent=t.title;element('task-workspace').textContent=(t.environment?.name?t.environment.name+' · ':'')+t.workspace;element('task-workspace').title=element('task-workspace').textContent;element('task-model').textContent=[taskEngineName(t.engine),t.model||'默认模型',effortLabels[t.reasoning_effort]||''].filter(Boolean).join(' · ');
...
async function loadCreateEnvironment(keepWorkspace=false){
 ...
 const directories=[...new Set([...env.workspaces,...tasks.filter(t=>t.environment?.id===env.id).map(t=>t.workspace)])];element('workspace-options').innerHTML=directories.map(p=>`<option value="${escapeHTML(p)}"></option>`).join('');if(!keepWorkspace)input('create-workspace').value=env.workspaces[0]||'';input('custom-model').value='';input('custom-model').classList.add('hidden');input('create-model').innerHTML='<option value="">正在读取模型列表…</option>';input('create-model').disabled=true;button('create-submit').disabled=true;element('models-hint').textContent='读取 '+env.name+' 的模型列表…';
 ...
 input('create-model').innerHTML='<option value="">使用此工具的默认模型</option>'+models.map(m=>`<option value="${escapeHTML(m.id)}">${escapeHTML(m.name)}</option>`).join('')+'<option value="__custom__">自定义模型…</option>';input('create-model').value=defaultModel||'';input('create-model').disabled=false;button('create-submit').disabled=false;element('models-hint').textContent=hint;updateReasoning();
}
