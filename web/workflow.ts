type WorkMode={id:string;name:string;permission:'workspace'|'read'|'full';approval?:'request'|'auto'|'never';prompt:string;builtin?:boolean;allow_network?:boolean};
...
function addCreateFiles(files:File[]){
 ...
 try{validateEngineAttachments(input('create-engine').value,files.length);validateAttachmentFiles(files,createFiles.length);createFiles.push(...files);element('create-error').textContent='';renderCreateFiles();notify('已添加 '+files.length+' 个附件，任务创建后会上传。')}
...
function setCreateSubmitState(state:'idle'|'starting'){
 ...
 const starting=state==='starting'||createSubmitting;submit.dataset.starting=starting?'1':'';
 element('create-form').querySelectorAll<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement|HTMLButtonElement>('input,select,textarea,button').forEach(control=>{
  if(control===submit)return;
  if(starting){if(control.dataset.submitDisabled===undefined)control.dataset.submitDisabled=String(control.disabled);control.disabled=true}
  else if(control.dataset.submitDisabled!==undefined){control.disabled=control.dataset.submitDisabled==='true';delete control.dataset.submitDisabled}
 });
...
function renderWorkflow(){
 ...
 const files=attachmentDrafts.get(chosen)||[],pending=pendingUploadFiles.get(chosen)||[];button('send').textContent=sending?'▶':'↑';button('send').title=sending?'正在开始':active?'追加要求并排队':'开始执行';button('send').setAttribute('aria-label',button('send').title);button('send').disabled=sending||uploadingTasks.has(chosen)||!!detail?.task.archived||harnessSessionClosed()||sessionResetTask===chosen||pending.length>0||(!input('message').value.trim()&&!files.length);
...
function selectedMessageMode(){const value=input('message-mode').value;return workCatalog.modes.some(m=>m.id===value&&modeSupportsEngine(m,detail?.task.engine))?value:''}
...
function editPreset(id:string){presetID=id;const item=(presetType==='modes'?workCatalog.modes:workCatalog.commands).find(m=>m.id===id),builtin=!!(item as WorkMode)?.builtin;input('preset-name').value=item?.name||'';input('preset-content').value=(item as WorkMode)?.prompt||(item as QuickCommand)?.content||'';input('preset-permission').value=(item as WorkMode)?.permission||'workspace';input('preset-approval').value=item&&presetType==='modes'?modeApproval(item as WorkMode):'request';input('preset-network').checked=(item as WorkMode)?.allow_network!==false;for(const id of ['preset-name','preset-content','preset-permission'])input(id).disabled=builtin;input('preset-network').dataset.builtin=builtin?'1':'';syncPresetNetwork();button('preset-save').disabled=builtin;button('preset-delete').disabled=builtin||!presetID;element('preset-error').textContent=builtin?'内置模式可直接选择；需要调整时新增一个模式。':''}
function presetNetworkForPermission(permission:string,id:string,current:boolean){return permission==='read'?id==='harness:read':permission==='full'?true:current}
function syncPresetNetwork(){const permission=input('preset-permission').value,network=input('preset-network'),approval=input('preset-approval');network.checked=presetNetworkForPermission(permission,presetID,network.checked);if(permission!=='workspace')approval.value='never';network.disabled=network.dataset.builtin==='1'||permission!=='workspace';approval.disabled=network.dataset.builtin==='1'||permission!=='workspace';element('preset-network-wrap').title=presetID==='harness:read'?'只读但不限制联网；此模式仅适用于 Harness':permission==='workspace'?'关闭后工作区模式不能下载依赖或访问 GitHub':'该执行方式由系统确定网络权限'}
...
async function changePreset(remove:boolean){button('preset-save').disabled=true;try{const fresh=await api<WorkCatalog>('workbench'),id=presetID||'custom_'+Date.now().toString(36);fresh.modes=fresh.modes.filter(m=>!m.builtin);if(presetType==='modes'){fresh.modes=fresh.modes.filter(m=>m.id!==id);if(!remove)fresh.modes.push({id,name:input('preset-name').value,permission:input('preset-permission').value as WorkMode['permission'],approval:input('preset-approval').value as WorkMode['approval'],prompt:input('preset-content').value,allow_network:input('preset-network').checked})}else{fresh.commands=fresh.commands.filter(c=>c.id!==id);if(!remove)fresh.commands.push({id,name:input('preset-name').value,content:input('preset-content').value})}workCatalog=await api<WorkCatalog>('workbench','PUT',fresh);modeOptions(element<HTMLSelectElement>('message-mode'),detail?.task.mode);modeOptions(element<HTMLSelectElement>('create-mode'));setCreatePermission(createPermission);renderPresetList();editPreset(remove?'':id);notify(remove?'预设已删除':'预设已保存')}catch(e){element('preset-error').textContent=(e as Error).message}finally{button('preset-save').disabled=false}}
...
async function confirmTrashTask(){const id=trashTaskID;button('trash-confirm').disabled=true;try{await api('tasks/'+id,'DELETE',{});element<HTMLDialogElement>('trash-confirm-dialog').close();tasks=await api<Task[]>('tasks');attachmentDrafts.delete(id);drafts.delete(id);if(id!==chosen){renderList();notify('已移入回收站');void loadStickyBoard();return}dirty=false;if(tasks.length){const next=tasks.find(t=>t.archived===(taskView==='archived'))||tasks[0];taskView=next.archived?'archived':'active';await choose(next.id)}else{chosen='';detail=null;resetConversation();element('conversation').innerHTML='<div class="empty"><h2>开始一个新任务</h2></div>';for(const name of ['tabs','task-actions','composer-wrap'])element(name).classList.add('hidden');element('task-title').textContent='Duo';element('task-workspace').textContent='';history.replaceState(null,'','/');switchTab('chat')}renderList();notify('已移入回收站');void loadStickyBoard()}catch(e){element('trash-confirm-error').textContent=(e as Error).message}finally{button('trash-confirm').disabled=false}}
