type WorkMode={id:string;name:string;permission:'workspace'|'read'|'full';prompt:string;builtin?:boolean;allow_network?:boolean};
...
function modeOptions(select:HTMLSelectElement,snapshot?:WorkMode){
 ...
 select.innerHTML=items.map(m=>`<option value="${escapeHTML(m.id)}">${escapeHTML(modeLabel(m))}</option>`).join('');select.value=items.some(m=>m.id===previous)?previous:snapshot?.id||'work';
}
...
function installWorkflow(){
 ...
 const permission=document.createElement('div');permission.className='create-permission';permission.innerHTML='<span>权限</span><div class="create-segmented" id="create-permission-options" role="group" aria-label="任务权限"><button type="button" data-create-permission="request" title="只读分析，不修改工作区">请求批准</button><button type="button" data-create-permission="auto" title="允许联网并自动处理工作区内改动">帮我批准</button><button type="button" data-create-permission="full" title="跳过工作区边界，仅在你明确选择时使用">完全访问</button></div><small id="create-permission-hint"></small>';
 ...
 element('create-permission-options').querySelectorAll<HTMLButtonElement>('[data-create-permission]').forEach(b=>b.onclick=()=>{
  ...
  setCreatePermission(value==='request'?'request':value==='full'?'full':'auto');
 });
 ...
 button('presets-close').onclick=()=>element<HTMLDialogElement>('presets-dialog').close();element('preset-form').onsubmit=savePreset;input('preset-permission').onchange=syncPresetNetwork;button('preset-delete').onclick=deletePreset;
...
function modeForPermission(permission:'request'|'auto'|'full'){
 const wanted=permission==='request'?'read':permission==='full'?'full':'workspace',preferred=permission==='request'?'plan':permission==='full'?'full':'work';
...
function setCreatePermission(permission:'request'|'auto'|'full'){
 ...
 if(element('create-permission-hint'))element('create-permission-hint').textContent=permission==='request'?'先分析并请求确认':permission==='full'?'跳过工作区边界，请确认任务来源可信':'允许联网并自动处理工作目录内的改动';
}
...
function editPreset(id:string){presetID=id;const item=(presetType==='modes'?workCatalog.modes:workCatalog.commands).find(m=>m.id===id),builtin=!!(item as WorkMode)?.builtin;input('preset-name').value=item?.name||'';input('preset-content').value=(item as WorkMode)?.prompt||(item as QuickCommand)?.content||'';input('preset-permission').value=(item as WorkMode)?.permission||'workspace';input('preset-network').checked=(item as WorkMode)?.allow_network!==false;for(const id of ['preset-name','preset-content','preset-permission'])input(id).disabled=builtin;input('preset-network').dataset.builtin=builtin?'1':'';syncPresetNetwork();button('preset-save').disabled=builtin;button('preset-delete').disabled=builtin||!presetID;element('preset-error').textContent=builtin?'内置模式可直接选择；需要调整时新增一个模式。':''}
function syncPresetNetwork(){const permission=input('preset-permission').value,network=input('preset-network');if(permission==='read')network.checked=false;if(permission==='full')network.checked=true;network.disabled=network.dataset.builtin==='1'||permission!=='workspace';element('preset-network-wrap').title=permission==='workspace'?'关闭后工作区模式不能下载依赖或访问 GitHub':'该执行方式由系统确定网络权限'}
...
async function changePreset(remove:boolean){button('preset-save').disabled=true;try{const fresh=await api<WorkCatalog>('workbench'),id=presetID||'custom_'+Date.now().toString(36);fresh.modes=fresh.modes.filter(m=>!m.builtin);if(presetType==='modes'){fresh.modes=fresh.modes.filter(m=>m.id!==id);if(!remove)fresh.modes.push({id,name:input('preset-name').value,permission:input('preset-permission').value as WorkMode['permission'],prompt:input('preset-content').value,allow_network:input('preset-network').checked})}else{fresh.commands=fresh.commands.filter(c=>c.id!==id);if(!remove)fresh.commands.push({id,name:input('preset-name').value,content:input('preset-content').value})}workCatalog=await api<WorkCatalog>('workbench','PUT',fresh);modeOptions(element<HTMLSelectElement>('message-mode'),detail?.task.mode);modeOptions(element<HTMLSelectElement>('create-mode'));renderPresetList();editPreset(remove?'':id);notify(remove?'预设已删除':'预设已保存')}catch(e){element('preset-error').textContent=(e as Error).message}finally{button('preset-save').disabled=false}}
