...
async function addAttachments(files:File[],task=chosen){if(!task||!files.length)return;if(uploadingTasks.has(task)){notify('请等待当前附件上传完成');return}try{const engine=detail?.task.id===task?detail.task.engine:tasks.find(item=>item.id===task)?.engine;validateEngineAttachments(engine,files.length);validateAttachmentFiles(files,(attachmentDrafts.get(task)||[]).length);uploadingTasks.add(task);renderWorkflow();for(const file of files){const uploaded=await uploadTaskFile(task,file);attachmentDrafts.set(task,[...(attachmentDrafts.get(task)||[]),uploaded]);if(task===chosen)renderWorkflow()}}catch(e){notify((e as Error).message)}finally{uploadingTasks.delete(task);if(task===chosen)renderWorkflow()}}
