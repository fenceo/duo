...
function modeOptions(select:HTMLSelectElement,snapshot?:WorkMode){
 const previous=select.value,items=[...workCatalog.modes],engine=select.id==='create-mode'?input('create-engine')?.value:detail?.task.engine;if(snapshot?.id&&!items.some(m=>m.id===snapshot.id))items.push(snapshot);
...
function modeSupportsEngine(mode:WorkMode,engine?:string){return engine!=='claude'||modeApproval(mode)!=='auto'}
...
function modeForPermission(permission:'request'|'auto'|'full'|'read'){
 ...
 const matches=(mode:WorkMode)=>mode.permission===wanted&&modeApproval(mode)===approval;
...
function setCreatePermission(permission:'request'|'auto'|'full'|'read'){
 const codex=input('create-engine')?.value!=='claude';if(permission==='auto'&&!codex)permission='request';
 ...
 element('create-permission-options')?.querySelectorAll<HTMLButtonElement>('[data-create-permission]').forEach(b=>{const on=b.dataset.createPermission===permission;b.classList.toggle('selected',on);b.setAttribute('aria-pressed',String(on));b.disabled=b.dataset.createPermission==='auto'&&!codex;if(b.dataset.createPermission==='request'){b.textContent=codex?'请求批准':'CLI 预授权';b.title=codex?'工作区内执行，需要提升权限时请求批准':'遵循 Claude CLI 预授权规则；不支持此页交互审批'}});
...
function renderWorkflow(){
 ...
 if(detail){const control=element<HTMLSelectElement>('message-mode'),key=chosen+'|'+detail.task.engine;if(control.dataset.task!==key){control.dataset.task=key;control.value='';modeOptions(control,detail.task.mode)}control.disabled=detail.task.archived;const hint=element('mode-engine-hint');hint.textContent='Claude CLI 预授权 · 当前不支持此页交互审批或 Codex 自动风险评审';hint.classList.toggle('hidden',detail.task.engine!=='claude');control.title=detail.task.engine==='claude'?hint.textContent:'本轮 Codex 工作模式与原生审批方式';}
...
function syncPresetNetwork(){const permission=input('preset-permission').value,network=input('preset-network'),approval=input('preset-approval');if(permission==='read')network.checked=false;if(permission==='full')network.checked=true;if(permission!=='workspace')approval.value='never';network.disabled=network.dataset.builtin==='1'||permission!=='workspace';approval.disabled=network.dataset.builtin==='1'||permission!=='workspace';element('preset-network-wrap').title=permission==='workspace'?'关闭后工作区模式不能下载依赖或访问 GitHub':'该执行方式由系统确定网络权限'}
