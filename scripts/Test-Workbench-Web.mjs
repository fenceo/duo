...
assert.match(workflowSource,/modelLabel=modelPicker\.previousElementSibling/);
...
const modelTask={id:'model-task',engine:'codex',model:'current-model',workspace:'C:\\work',reasoning_effort:'',environment:{id:'model-env',name:'Windows',type:'windows',host:'',distro:'',user:'',model:'configured-model',claude_model:'configured-claude',workspaces:[]}};
...
const classList=()=>{const values=new Set(['hidden']);return {contains:value=>values.has(value),add:value=>values.add(value),remove:value=>values.delete(value),toggle(value,on){on?values.add(value):values.delete(value)}}};
const nodes=new Map();
...
await execution.toggleModelMenu('task');await execution.toggleModelMenu('task');await execution.toggleModelMenu('task');await execution.toggleModelMenu('task');await new Promise(resolve=>setImmediate(resolve));
assert.match(nodes.get('task-models-hint').textContent,/模型目录读取失败/);
assert.match(nodes.get('task-models-status').textContent,/可刷新或输入模型 ID/);
