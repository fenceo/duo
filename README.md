...
从 0.20 起，安装向导明确区分三种数据选择：

- **保留当前数据**：升级默认选项，原任务、密码及配置继续使用。自动更新只能使用此模式。
- **全新配置**：选择空目录或不存在的新目录，首次启动再设置密码及执行环境。原数据保留，不删除、不复制到新目录。
- **载入已有数据**：选择一套已正常退出的 Duo 数据目录，先只读校验 `config.json`、数据库结构和初始化状态；沿用那套数据的密码及配置，不合并两套任务。

不想在安装时配置环境，可以先用默认数据位置完成安装，首次启动时再配置。安装后也可以在“设置 → 数据与存储”中切换到一套已完整复制、正常退出的已有数据目录：Duo 会先确认任务、终端和硬件空闲，再由启动器安全重启；原目录不会删除，也不会自动合并。目标目录需与当前使用相同的监听端口。若使用安装向导，也可以在数据选择页完成同样的载入，不需要先卸载。已有程序目录保持锁定，以免产生两个相互争抢入口的安装。

不要只复制一个 `jianzuo.db`，也不要为通过校验而删除 `-wal` 或日志文件。完整备份须在原程序正常退出后进行。数据加载不会升级或重建 WSL、AI CLI、SSH 密钥及引擎账号。

跨电脑或跨数据目录搬运任务时，请使用“设置 → 数据与存储 → 脱敏工作区导出 / 导入”。该 ZIP 只包含任务、对话、执行记录、知识、待办和备注，导入后追加为新任务并绑定当前电脑环境；不会携带密码、登录会话、飞书/AI/SSH/硬件凭据、机器路径或附件原文。完整数据目录切换仅适合同一台电脑的本地恢复。
