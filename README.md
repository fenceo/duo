...
也可以下载 `Duo-portable-windows-x64.zip`，完整解压后双击 **Duo.exe**。首次只需设置面板密码；程序进入工作台后，再在“设置”中添加执行环境、AI 工具、工作目录、网络访问或 SSH。程序驻留 Windows 系统托盘，可打开面板、查看日志、重启或退出。
...
- 模型选择器按环境和引擎读取模型：Codex 使用当前账号的原生目录，Claude 读取明确配置，Harness 列出 Duo 配置项。“重读模型列表”和需确认的“测试列表模型”集中在选择器中；逐项显示调用结果，允许停止测试。列表本身不等同于余额或调用成功验证。
...
安装器会先探测当前用户是否已有 Duo 安装、注册的启动任务以及可识别的旧便携入口：

- **首次干净安装**：只显示数据目录（默认 `%LOCALAPPDATA%\Duo\data`），无需再选择“数据模式”；下一步即可安装，首次启动只设置密码，进入工作台后再配置执行环境。
- **已有安装升级**：自动沿用登记的程序目录、数据目录、桌面入口和登录自启，不再重复询问；安装不会删除或重建旧 data。
- **检测到旧便携版**：显示检测到的数据路径，迁移页只确认启动任务和快捷方式接管，不复制、合并或删除旧程序和旧 data。

需要在同一台电脑切换完整 data 时，使用“设置 → 数据与存储”的完整目录切换；它会先确认任务、终端和硬件空闲，再由启动器安全重启，原目录始终保留。需要全新配置可在设置中选新空目录，或退出 Duo 后将旧 data 移到备份位置再启动。跨电脑或安装版与便携版互导请使用“脱敏工作区导出 / 导入”，不要直接复制正在使用的 SQLite 文件。

不要只复制一个 `jianzuo.db`，也不要为通过校验而删除 `-wal` 或日志文件。完整备份须在原程序正常退出后进行。数据加载不会升级或重建 WSL、AI CLI、SSH 密钥及引擎账号。

跨电脑或跨数据目录搬运任务时，请使用“设置 → 数据与存储 → 脱敏工作区导出 / 导入”。该 ZIP 包含任务、对话、执行记录、知识、待办、备注、自定义工作模式和快捷指令，导入后追加为新任务并绑定当前电脑环境；不会携带密码、登录会话、飞书/AI/SSH/硬件凭据、机器路径或附件原文。完整数据目录切换仅适合同一台电脑的本地恢复。
